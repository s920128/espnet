...
from espnet.nets.pytorch_backend.med.decoder_layer import DecoderLayer
...
class Decoder(BatchScorerInterface, torch.nn.Module):
    ...
    def forward(self, tgt, tgt_mask, memory, memory_mask, memory2, memory_mask2):
        """Forward decoder.
        ...
        """
        x = self.embed(tgt)
        x, tgt_mask, memory, memory_mask, memory2, memory_mask2 = self.decoders(
            x, tgt_mask, memory, memory_mask, memory2, memory_mask2
        )
        if self.normalize_before:
            x = self.after_norm(x)
    ...
    def forward_one_step(self, tgt, tgt_mask, memory, memory2,cache=None):
        """Forward one step.
    ...
    def score(self, ys, state, x, x2):
        """Score."""
        ys_mask = subsequent_mask(len(ys), device=x.device).unsqueeze(0)
        if self.selfattention_layer_type != "selfattn":
            state = None
        logp, state = self.forward_one_step(
            ys.unsqueeze(0), ys_mask, x.unsqueeze(0), x2.unsqueeze(0), cache=state
        )
    ...
    def batch_score(
        ...
        logp, states = self.forward_one_step(ys, ys_mask, xs, xs2, cache=batch_state)
