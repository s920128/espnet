...
class DecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        if self.concat_after:
            ...
            self.concat_linear22 = nn.Linear(size + size, size)

    def forward(self, tgt, tgt_mask, memory, memory_mask, memory2, memory_mask2, cache=None):
        ...
        if self.concat_after:
            ...
            x_concat2 = torch.cat(
                (x, self.src_attn(x, memory2, memory2, memory_mask2)), dim=-1
            )
        ...
        return x, tgt_mask, memory, memory_mask, memory2, memory_mask2
